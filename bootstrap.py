...
SLUG = 'jewish-historical-migration'
...
def modify_angular_json():
    with open('frontend/angular.json', 'r') as file:
        data = json.load(file)
    try:
        project = 'jewish-historical-migration'.replace('_', '-')
...
def activate_frontend():
    ...
    if framework == 'backbone':
        ...
        project_name = 'jewish-historical-migration'.replace('_', '-')
        Command(
        ...
        Command(
            'Creating project',
            ['yarn', 'ng', 'new', project_name, '--prefix=jhm',
                '--skip-git=true',
                '--skip-install=true',
        ...
        if '4200' != '4200':
            Command(
                'Set frontend port',
                ['yarn', 'ng', 'config', "projects.jewish-historical-migration.architect.serve.options.port", '4200'],
                cwd="frontend"
            )()
    else:
