...
class PlaceManager(models.Manager):
    def create_place(self, row_dict, location_sheet):
        area, region = None
        if row[1] != None:
            area = Area.objects.get_or_create(name=row_dict['area'])
        if row[2] != None:
            region = Region.objects.get_or_create(name=row['province-region'])
        place = self.get_or_create(
            name=row['placename'],
            area=area,
            region=region,
        )
        place.pleiades_id = row['pleiades']
        if place.pleiades_id:
            coordinates = self.fetch_from_pleiades()
        else:
            coordinates = self.fetch_from_document(location_sheet, row['own id'])
        if coordinates:
    ...
    def fetch_from_document(self, location_sheet, identifier):
        ...
        '''
        return next(((row[4], row[5]) for row in location_sheet.values if row[0] == identifier), None)
...
class Record(models.Model):
    ...
    pleiades_place = models.ForeignKey(to=PleiadesPlace, null=True, blank=True, on_delete=models.SET_NULL)
    ...
    def import_dataset(self, input_file):
        ...
        for index, row in enumerate(sheet.values):
            if index == 0:
                keys = [cell for cell in row[:18]]
                continue
            row_dict = {k: row[i] for i, k in enumerate(row)}
            Place.objects.create_place(
                row_dict, sheet2
            )
